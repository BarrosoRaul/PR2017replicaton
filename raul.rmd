...
```{r}
    length(unique(pharmaco$cellLine))
```
...
```{r}
    ggplot(summarizedPharmacoData, aes(auc_CCLE, auc_GDSC)) + geom_point()
```

2. Calculate correlation coefficients of the AUC in GDSC and CCLE for each drug (hint: code from Tutorial 2 may help). 

```{r}
    cor(summarizedPharmacoData$auc_CCLE, summarizedPharmacoData$auc_GDSC)
```
...
```{r}
    ggplot(summarizedPharmacoData, aes(ic50_CCLE, ic50_GDSC)) + scale_y_log10() + scale_x_log10() + geom_point()
    cor(summarizedPharmacoData$ic50_CCLE, summarizedPharmacoData$ic50_GDSC)
    #AUC
```
